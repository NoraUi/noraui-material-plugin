...
if [ "$TRAVIS_REPO_SLUG" == 'NoraUi/noraui-material-plugin' ] && [ "$TRAVIS_BRANCH" == 'master' ] && [ "$TRAVIS_PULL_REQUEST" == 'false' ] && [ "$AUTHOR_NAME" != 'Travis-CI' ] && [ "$BUILD_LEADER" == 'YES' ] && [ "$BUILD_AGGREGATE_STATUS" == 'others_succeeded' ]; then
